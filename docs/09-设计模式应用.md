...
本酒店管理系统采用了7种经典设计模式，这些模式的合理运用不仅提高了代码的可维护性和扩展性，还体现了面向对象设计的核心思想。每个设计模式都解决了特定的设计问题，共同构建了一个灵活、健壮的系统架构。
...
### 设计目的
在请求到达目标资源之前或响应返回客户端之前，对请求和响应进行拦截和处理，实现横切关注点的统一管理。

### 应用场景
1. **LoginFilter** - 登录验证过滤器
...
本酒店管理系统通过合理运用7种设计模式，构建了一个结构清晰、功能完善、易于维护和扩展的系统架构：
